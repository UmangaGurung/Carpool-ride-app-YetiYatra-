...
class _MapPageState extends State<MapPage> {
  ...
  void _sendRideRequest() async {
    ...
    Map<String, dynamic> data = {
      ...
      'phone': widget.phone,
      'rider': widget.result['username'],
      'riderphone': widget.result['phone'],
      'leaving': widget.result['leaving'],
      'destination': widget.result['destination'],
      'scheduleid': widget.result['scheduleid'],
      'noOfpassengers': widget.noOfpassengers,
    };

    try {
      final response = await http.post(
        Uri.parse('$url/rideRequest'),
      ...
      if (response.statusCode == 201) {
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return WillPopScope(
              ...
              child: AlertDialog(
                title: Text('Failed'),
                content: Text('You have a ride request pending'),
                actions: <Widget>[
                  TextButton(
                    child: Text('OK'),
                    onPressed: () {
        ...
        print('Failed to request: ${response.statusCode}');
